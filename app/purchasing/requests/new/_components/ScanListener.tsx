...
import { getItemIdFromLot } from "../_functions/getItemIdFromLot";
...
const ScanListener = ({ handleItemSelection }: { handleItemSelection: (item: string) => void; }) => {
    ...
    return (
        <div className="p-10 rounded-lg border-2 border-neutral-800 flex flex-col items-center justify-center gap-y-4">
            {!isLoading && (
                <>
                    <BsQrCodeScan className="text-[200px]" />
                    <h1 className="font-poppins text-4xl font-bold">Scan Lot Barcode</h1>
                </>
            )}
            ...
            {isLoading && (
                <div role="status">
                    <svg
                        aria-hidden="true"
                        className="w-40 h-40 text-gray-200 animate-spin dark:text-gray-600 fill-lilac-600"
                        viewBox="0 0 100 101"
                        fill="none"
